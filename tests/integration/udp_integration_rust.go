...
func main() {
	if len(os.Args) > 1 && os.Args[1] == "-help" {
		fmt.Println("Prismux Integration Test Tool")
		fmt.Println("Usage: go run udp_test.go")
		fmt.Println("This tool tests Prismux configurations by sending UDP packets and measuring performance")
		return
	}
	...
	// Build Prismux binary
	fmt.Println("Building Prismux...")
	if err := buildPrismux(projectRoot); err != nil {
		fmt.Printf("Failed to build Prismux: %v\n", err)
		os.Exit(1)
	}
...
func getProjectRoot() string {
	dir, _ := os.Getwd()
	for {
		// Look for Prismux project indicators.
...
func buildPrismux(projectRoot string) error {
	binaryName := "prismux"
	// Add .exe extension on Windows
	if filepath.Ext(os.Args[0]) == ".exe" {
		binaryName += ".exe"
	}
	binaryPath := filepath.Join(projectRoot, "target", "release", binaryName)

	cmd := exec.Command("cargo", "build", "--release", "--bin", "prismux")
	cmd.Dir = projectRoot
	cmd.Env = append(os.Environ(), "RUSTFLAGS=-A dead_code")
	...
	if _, err := os.Stat(binaryPath); err != nil {
		return fmt.Errorf("build ok but binary not found at %s", binaryPath)
	}
...
func runTest(projectRoot, examplesDir string, config TestConfig, label string, withSleep bool) TestResult {
	...
	for _, configFile := range config.ConfigFiles {
		...
		process := startPrismuxProcess(projectRoot, configPath)
		if process == nil {
			result.Error = "Failed to start Prismux process"
			return result
		}
...
func startPrismuxProcess(projectRoot, configPath string) *exec.Cmd {
	binaryName := "prismux"
	// Add .exe extension on Windows
	if filepath.Ext(os.Args[0]) == ".exe" {
		binaryName += ".exe"
	...
	if err := cmd.Start(); err != nil {
		fmt.Printf("Failed to start Prismux: %v\n", err)
		return nil
	}
	...
	case msg := <-failCh:
		...
		logs := logBuf.String()
		logMu.Unlock()
		fmt.Printf("Prismux failed to start for config %s: %s\n--- Logs ---\n%s\n------------\n", configPath, msg, logs)
	...
	case <-time.After(timeout):
		...
		logs := logBuf.String()
		logMu.Unlock()
		fmt.Printf("Prismux did not signal readiness within %v for config %s\n--- Logs ---\n%s\n------------\n", timeout, configPath, logs)
		if cmd.Process != nil {
			_ = cmd.Process.Kill()
		}
		return nil
